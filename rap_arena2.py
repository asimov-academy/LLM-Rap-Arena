...
server_url = "http://192.168.1.4:11434"
...
# llm2 = ChatOllama(model="taozhiyuai/llama-3-8b-lexi-uncensored:q8_0", temperature=0.5)
llm2 = ChatOllama(server_url, model="taozhiyuai/llama-3-8b-lexi-uncensored:f16", temperature=0)
# llm2 = ChatOllama(model="gurubot/alpha-orionis-uncensored", temperature=0)
# llm2 = ChatOllama(model="dolphin-phi:2.7b-v2.6-q8_0", temperature=0)
# llm2 = ChatOllama(model="dolphin-mistral:7b-v2-q8_0", temperature=0)
# llm2 = ChatOllama(model="gfg/solar-10.7b-instruct-v1.0-uncensored", temperature=0)
# llm2 = ChatOllama(model="dolphin-mixtral:latest", temperature=0)


# llm2 = ChatOllama(model="gdisney/mistral-uncensored", temperature=0)
...
system_template = """
    ...
    Seu nome é: {who_iam}.
    Seu adversário se chama: {opponent}.
    Lembre-se que se rimar usando o nome do oponente ganhará pontos 
    com o público.
    
    Seja agressivo, fale palavrões, xingue o pai!
    Se você não receber nenhuma entrada, inicie rimando.

    Pegue a última frase do seu oponente e use-a como inspiração para
    sua próxima rima.

    Sua rima deve ter 1 estrofe com 4 frases.
    Use frases curtas.  
"""

base_prompt = """
    Histórico da batalha: {chat_history}
